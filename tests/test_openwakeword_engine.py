...
class TestOpenWakeWordEngine(
    ...
    def test_start_loads_expected_onnx_model(
        self,
    ) -> None:
        self.engine.start()
        ...
        self.assertEqual(
            self.fake_model.reset_count,
            1,
        )

        self.assertEqual(
            self.fake_model.predict_count,
            DEFAULT_WARMUP_FRAMES,
        )
    ...
    def test_start_requires_model_file(
        self,
    ) -> None:
        missing_model = (
            OpenWakeWordEngine(
    ...
    def test_start_requires_external_data_file(
        self,
    ) -> None:
    ...
    def test_audio_is_converted_to_int16_samples(
        self,
    ) -> None:
        self.engine.start()
    ...
    def test_wrong_audio_frame_size_fails(
        self,
    ) -> None:
        self.engine.start()
    ...
    def test_non_bytes_audio_fails(
        self,
    ) -> None:
        self.engine.start()
    ...
    def test_paused_engine_ignores_audio(
        self,
    ) -> None:
    ...
    def test_resume_resets_and_rewarms_model(
        self,
    ) -> None:
        self.engine.start()
    ...
    def test_warmup_frames_must_not_be_negative(
        self,
    ) -> None:
        with self.assertRaises(
            ValueError
